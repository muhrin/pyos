...
points to the parent directory and the destination points to the file or directory contained
within it.
...
class Schema:
    ...
    # Optional fields
    STIME = (
        mincepy.SNAPSHOT_TIME
    )  # Last snapshot time of the object (i.e. last time it was changed)
    DESCENDENTS = DESCENDENTS
    DEPTH = "depth"
    VER = mincepy.VERSION
...
def path_part_lookup(name: str) -> dict:
    """
    Returns a MongoDB aggregate stage that does a graph lookup for a particular path part by
    matching the name
    """
    return {
        "$graphLookup": {
            ...
            "as": "children",
            "maxDepth": 0,
            "restrictSearchWithMatch": {Schema.NAME: name},
        },
    }
...
def _path_lookup(path: Path, start_id=Entry.id(ROOT)) -> list[dict]:
    """
    Returns a list of MongoDB aggregate stage operations that will match a filesystem path based
    on the passed parts
    """
    validate_path(path, absolute=start_id == Entry.id(ROOT))

    aggregate = [
        {"$match": {Schema.ID: start_id}},
        {"$addFields": {Schema.PATH_ENTRIES: ["$$ROOT"]}},  # Add start document to the path
    ]
    for part in path[1:]:
        aggregate.extend(
            [
...
def _records_lookup() -> list[dict]:
    ...
    return [
        ...
        # Don't keep object entries that have no records,
        # these are spurious and should be cleaned up
        {"$match": {"$or": [{Schema.TYPE: Schema.TYPE_DIR}, {RECORDS: {"$ne": []}}]}},
        # Now keep certain fields that we're interested in
        {
            "$addFields": {
                Schema.CTIME: {"$arrayElemAt": [f"$records.{mincepy.mongo.db.CREATION_TIME}", 0]},
                Schema.STIME: {"$arrayElemAt": [f"$records.{mincepy.mongo.db.SNAPSHOT_TIME}", 0]},
                Schema.VER: {"$arrayElemAt": [f"$records.{mincepy.mongo.db.VERSION}", 0]},
                Schema.TYPE_ID: {"$arrayElemAt": [f"$records.{mincepy.mongo.db.TYPE_ID}", 0]},
                RECORDS: "$$REMOVE",
            }
        },
    ]
...
def find_entry(path: Path, *, session: Optional["pyos.Session"] = None) -> Optional[dict]:
    ...
    assert len(res) == 1, (
        f"It should never happen that there is more than one match for a particular path but "
        f"got: {res}"
    )
    entry = res[0]
    ...
    if Schema.ID not in entry:
        ...
        # This code can be reached, for example, when all but the last path part exists
        return None
    ...
    def _set_path_operations(
        ...
        if only_new:
            ...
            update = {
                ...
                "$setOnInsert": {Schema.TYPE: Schema.TYPE_OBJ, Schema.CTIME: time_now},
            }

        return [
            pymongo.UpdateOne(
                {Schema.ID: obj_id, Schema.TYPE: Schema.TYPE_OBJ}, update, upsert=True
            )
        ]
...
def rename(
    ...
    try:
        res = coll.update_one(
            {Schema.ID: src_id},
            {"$set": {Schema.PARENT: Entry.id(new_dir), Schema.NAME: basename}},
            upsert=False,
        )
