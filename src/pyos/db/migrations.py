...
def initial(historian: mincepy.Historian):
    ...
    historian.meta.create_index(
        [(config.NAME_KEY, mincepy.ASCENDING), (config.DIR_KEY, mincepy.ASCENDING)],
        unique=True,
        where_exist=True,
    )
...
def add_pyos_collections(historian: mincepy.Historian):
    ...
    if schema_version >= 2:
        meta_entries = archive.data_collection.aggregate(
            [{"$match": {"meta": {"$exists": True}}}, {"$replaceRoot": {"newRoot": "$meta"}}]
        )
    else:
    ...
    fs_collection.create_index(
        [(fs.Schema.PARENT, mincepy.ASCENDING), (fs.Schema.NAME, mincepy.ASCENDING)], unique=True
    )
