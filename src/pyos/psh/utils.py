...
class ThreadStreamRedirector(io.TextIOBase):
    ...
    def close(self):
        ...
        if stream.name == "<stdin>":
            logging.warning(
                "Someone is closing the standard input: \n%s",
...
class Piper:
    ...
    def _run_func(self, func, open_in_stream, open_out_stream, done_redirecting: threading.Event):
        ...
        with open_in_stream as in_stream, open_out_stream as out_stream:
            with (
                self._in_redir.redirect(in_stream),
                self._out_redir.redirect(out_stream),
            ):
...
class RedirectionSavedState(cmd2.utils.RedirectionSavedState):
    ...
    def __init__(
        self,
        self_stdout: Union[cmd2.utils.StdSim, TextIO],
        sys_stdout: Union[cmd2.utils.StdSim, TextIO],
        self_stdin: TextIO,
        sys_stdin: TextIO,
        pipe_proc_reader: Optional[cmd2.utils.ProcReader],
