...
class PyosShell(cmd2.Cmd):
    ...
    def do_shell(self, args: argparse.Namespace) -> None:
        ...
        with self.sigint_protection:
            ...
            kwargs = dict(
                ...
                stdout=(
                    subprocess.PIPE if isinstance(self.stdout, cmd2.utils.StdSim) else self.stdout
                ),
                stderr=(
                    subprocess.PIPE if isinstance(sys.stderr, cmd2.utils.StdSim) else sys.stderr
                ),
                shell=True,  # nosec
            )

            _LOGGER.debug(
                "Starting shell command: %s. Capturing stdin: %s",
                expanded_command,
                self._redirecting,
            )
            with subprocess.Popen(expanded_command, **kwargs) as proc:  # nosec
        ...
        def do_ipy(self, _: argparse.Namespace) -> Optional[bool]:
            ...
            def load_ipy(cmd2_app: PyosShell, py_bridge: PyBridge):
                ...
                if cmd2_app.self_in_py:
                    exec("self = cmd2_app")  # pylint: disable=exec-used # nosec
                ...
                cmd2.cmd2.embed(
                    banner1=(
                        "Entering an pyOS IPython shell. Type quit or <Ctrl>-d to exit.\n"
    ...
    def _create_redirection_save(self):
        ...
        return utils.RedirectionSavedState(
            self.stdout,
            sys.stdout,
            self.stdin,
            sys.stdin,
    ...
    def _restore_output(
        self, statement: cmd2.Statement, saved_redir_state: utils.RedirectionSavedState
    ):
