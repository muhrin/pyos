...
@psh_lib.flag(flags.f, help="force - do not prompt before overwriting")
def mv(options, *args):  # pylint: disable=invalid-name
    ...
    args = list(args)
    dest = pathlib.Path(args.pop())

    to_move = ls(-flags.d, *args)
    ...
    if dest.is_dir():
        ...
        # Renaming
        if dest.exists():
            if -flags.f in options or click.confirm(f"Overwrite '{dest}'?"):
                dest.unlink()
            else:
                return
