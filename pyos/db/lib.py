from typing import Sequence, Iterable, Any, Optional, Tuple
...
__all__ = ('init', 'reset', 'get_meta', 'update_meta', 'set_meta', 'set_name', 'save_one',
           'save_many')
...
def save_one(obj, path: pyos.fs.PathSpec = None, overwrite=False):
...
def save_many(to_save: Iterable[Tuple[Any, pyos.fs.PathSpec]], overwrite=False):
    """
...
def find(
    ...
    if starting_point:
        spoints = [str(pyos.PyosPath(path).resolve()) for path in starting_point]
    else:
        spoints = [str(pyos.fs.cwd())]
